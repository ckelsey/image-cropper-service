...
function getUploadProperties(data, params, file) {
	...
	var result = {}
	result.index = data.partindex ? parseInt(sanitize.number(data.partindex[0])) : 0
	result.totalSize = data.totalfilesize ? parseInt(sanitize.number(data.totalfilesize[0])) : 0
	result.totalParts = data.totalparts ? parseInt(sanitize.number(data.totalparts[0])) : 1
	result.done = sanitize.number(params.done) || true
	result.id = data.id ? sanitize.string(data.id[0]) : "anid"
	console.log(result);
	// if (
	// 	!result.totalSize ||
	// 	!result.totalParts ||
...
module.exports = function (headers, files, body, query, uploadPath) {
	return new Promise((resolve, reject) => {
		...
		properties.type = fileType(readChunk.sync(file, 0, 4100));
